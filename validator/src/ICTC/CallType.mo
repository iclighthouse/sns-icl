...
module {
	...
	public type CallType = {
		...
		#ICRC1 : {
			// ICRC1 -> ICRC1New result  (Old version ICRC1)
			#icrc1_transfer : (ICRC1.TransferArgs);
			#icrc1_balance_of : (ICRC1.Account);
		};
		#ICRC1New : {
			// ICRC1New -> ICRC1New result
	...
	public type Receipt = {
		...
		#This : {
			...
			// #dip20SendComp : ();
			#batchTransfer : ([{
				token0 : { locked : Nat; available : Nat };
				token1 : { locked : Nat; available : Nat };
			}]);
		...
		switch (_domain) {
			...
			case (#Local(localCall)) {
				switch (_args) {
					...
					case (#__block) {
						return (#Error, ? #__block, ?{ code = #future(9904); message = "Blocked by code." });
					};
					case (#This(method)) {
						try {
					...
					case (_) {
						return (#Error, null, ?{ code = #future(9901); message = "No such actor." });
					};
				};
			};
					...
					case (#DRC20(method)) {
						...
						switch (method) {
							...
							case (#approve(spender, amount, nonce, sa, data)) {
								var result : DRC20.TxnResult = #err({
									code = #UndefinedError;
									message = "No call.";
								}); // Receipt
								try {
									...
									switch (result) {
										case (#Ok(txid)) {
											return (#Done, ? #DRC20(#approve(result)), null);
										};
										case (#err(e)) {
											return (#Error, ? #DRC20(#approve(result)), ?{ code = #future(9903); message = e.message });
										};
									};
								} catch (e) {
									return (#Error, null, ?{ code = Error.code(e); message = Error.message(e) });
							...
							case (#transfer(to, amount, nonce, sa, data)) {
								...
								try {
									...
									switch (result) {
										case (#Ok(txid)) {
											return (#Done, ? #DRC20(#transfer(result)), null);
										};
										case (#err(e)) {
											return (#Error, ? #DRC20(#transfer(result)), ?{ code = #future(9903); message = e.message });
										};
									};
								} catch (e) {
									return (#Error, null, ?{ code = Error.code(e); message = Error.message(e) });
							...
							case (#transferFrom(from, to, amount, nonce, sa, data)) {
								...
								try {
									...
									switch (result) {
										case (#Ok(txid)) {
											return (#Done, ? #DRC20(#transferFrom(result)), null);
										};
										case (#err(e)) {
											return (#Error, ? #DRC20(#transferFrom(result)), ?{ code = #future(9903); message = e.message });
										};
									};
								} catch (e) {
									return (#Error, null, ?{ code = Error.code(e); message = Error.message(e) });
							...
							case (#lockTransfer(to, amount, timeout, decider, nonce, sa, data)) {
								...
								try {
									...
									switch (result) {
										case (#Ok(txid)) {
											return (#Done, ? #DRC20(#lockTransfer(result)), null);
										};
										case (#err(e)) {
											return (#Error, ? #DRC20(#lockTransfer(result)), ?{ code = #future(9903); message = e.message });
										};
									};
								} catch (e) {
									return (#Error, null, ?{ code = Error.code(e); message = Error.message(e) });
							...
							case (#lockTransferFrom(from, to, amount, timeout, decider, nonce, sa, data)) {
								...
								try {
									...
									switch (result) {
										case (#Ok(txid)) {
											return (#Done, ? #DRC20(#lockTransferFrom(result)), null);
										};
										case (#err(e)) {
											return (#Error, ? #DRC20(#lockTransferFrom(result)), ?{ code = #future(9903); message = e.message });
										};
									};
								} catch (e) {
									return (#Error, null, ?{ code = Error.code(e); message = Error.message(e) });
							...
							case (#executeTransfer(txid, executeType, to, nonce, sa, data)) {
								...
								try {
									...
									switch (result) {
										case (#Ok(txid)) {
											return (#Done, ? #DRC20(#executeTransfer(result)), null);
										};
										case (#err(e)) {
											return (#Error, ? #DRC20(#executeTransfer(result)), ?{ code = #future(9903); message = e.message });
										};
									};
								} catch (e) {
									return (#Error, null, ?{ code = Error.code(e); message = Error.message(e) });
					...
					case (#ICRC1(method)) {
						...
						switch (method) {
							...
							case (#icrc1_transfer(args)) {
								var result : {
									#Ok : Nat;
									#Err : ICRC1New.TransferError;
								} = #Err(#TemporarilyUnavailable); // Receipt
								try {
									// do
									...
									switch (result) {
										case (#Ok(id)) {
											return (#Done, ? #ICRC1New(#icrc1_transfer(result)), null);
										};
										case (#Err(e)) {
											return (#Error, ? #ICRC1New(#icrc1_transfer(result)), ?{ code = #future(9903); message = "ICRC1 token Err." });
										};
									};
								} catch (e) {
									return (#Error, null, ?{ code = Error.code(e); message = Error.message(e) });
					...
					case (#ICDex(method)) {
						...
						switch (method) {
							...
							case (#trade_b(_order, _orderType, _expiration, _nonce, _sa, _data, _brokerage)) {
								var result : ICDex.TradingResult = #err({
									code = #UndefinedError;
									message = "No call.";
								}); // Receipt
								try {
									...
									switch (result) {
										case (#Ok(res)) {
											return (#Done, ? #ICDex(#trade_b(result)), null);
										};
										case (#err(e)) {
											return (#Error, ? #ICDex(#trade_b(result)), ?{ code = #future(9903); message = "Calling Err." });
										};
									};
								} catch (e) {
									return (#Error, null, ?{ code = Error.code(e); message = Error.message(e) });
							...
							case (#tradeMKT_b(_token, _value, _limitPrice, _nonce, _sa, _data, _brokerage)) {
								...
								try {
									...
									switch (result) {
										case (#Ok(res)) {
											return (#Done, ? #ICDex(#tradeMKT_b(result)), null);
										};
										case (#err(e)) {
											return (#Error, ? #ICDex(#tradeMKT_b(result)), ?{ code = #future(9903); message = "Calling Err." });
										};
									};
								} catch (e) {
									return (#Error, null, ?{ code = Error.code(e); message = Error.message(e) });
